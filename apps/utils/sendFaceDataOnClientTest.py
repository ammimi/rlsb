...
def sendLocalImage(server_url, params, image_path):
    f = open(image_path,"rb")
    img_raw_data = f.read()
    f.close()
    base64_content_string = base64.b64encode(img_raw_data)
...
if __name__ == "__main__":
   ...
   #url = "http://192.168.1.219/cgi-bin/v0.13/facemng/add.py"
   #url = "http://192.168.1.219/v0.13/facemng/add"
   url =  "http://222.128.127.3:8185/v0.13/facemng/add"
   ...
   """
   faceName = "liuguoqiang"
   faceCName = "刘国强"
   imageFileExtention = "jpg"
   file_path = "D:\\pythoncodes\\faceRecogAppWithWebCam\\known_imgs\\liuguoqiang.jpg"
   """
   ...
   faceCName = "张凯丽"
   imageFileExtention = "jpeg"
   file_path = "D:\\pythoncodes\\facerecognapp\\known_imgs\\zhangkaili.jpeg"
   """
   
   faceName = "wangfei"
   faceCName = "王菲"
   imageFileExtention = "jpg"
   # file_path = "D:\\pythoncodes\\facerecognapp\\known_imgs\\wangfei.jpeg"
   file_path = "/home/mummywho/virtualenv/rlsb/media/attendance/2018/12/timg_7pRiOkh.jpg"
   
   params = {"client_id":clientId, "client_secret":clientSecret,"face_name":faceName,"face_cname":faceCName,"timestamp":timestamp,"image_file_extention":imageFileExtention}

   paramNamesForSignature = sorted(["client_secret","face_name","client_id","timestamp","image_file_extention"],key=str.lower)
